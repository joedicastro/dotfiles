...
- [[http://geoff.greer.fm/2011/12/27/the-silver-searcher-better-than-ack][Ag ("The Silver Searcher")]] or [[https://github.com/BurntSushi/ripgrep][rg ("ripgrep")]] or [[https://github.com/monochromegane/the_platinum_searcher][pt ("platinum searcher")]]
...
This another code is to provide a hook to do some things after loading a theme,
like overwriting a face in a particular theme or always reload the
...
** Search the internet from Emacs

This function uses DuckDuckGo to search the web from Emacs. The result can be
shown in a browser or in ewww. By default the bang (!) is included in the search
but can be deleted to search directly in DuckDuckGo.
...
[[https://github.com/xuchunyang/DevDocs.el][devdocs]] is an Emacs package allowing you to easily search the [[https://devdocs.io][DevDocs]]
documentation.
...
[[https://github.com/d12frosted/flyspell-correct][flyspell-correct-ivy]] correct words with flyspell via custom interface (ivy)
...
+ Interface for commands:
  I use the most suited package for this job, Ivy, that is the quasi-equivalent
...
+ Activating through the "\" key to call all of the general and by-package
  menus. Using this, and occasionally the =counsel-descbinds= command (C-h b), I
...
#+BEGIN_SRC emacs-lisp
  ...
  --------------------------------------------------------------------------------
    ...
    Switch                 Do                                          │ Buffers │
  ╭────────────────────────────────────────────────────────────────────┴─────────╯
    [_b_] switch (ivy)       [_d_] kill the buffer
  ...
  ╭───────────────────────────────────────────────────────────────────┴──────────╯
    [^c^] commands history       [^e^] emacs registers    [_u_] undo tree
  ...
  ╭───────────────────────────────────────────────────────────────┴──────────────╯
    ...
    [_d_] calendar    [^s^] search headings     [_h_] add location
...
This load the =mail.el.gpg= file where I store the email information that I
prefer to keep private (since I share this config in GitHub) to inquisitive
eyes. I only load it once per session, is less secure but faster.

#+BEGIN_SRC emacs-lisp
  (defvar joe-mu4e-user-info nil
    "This var is to only load this info once.")
  (add-hook 'mu4e-main-mode-hook (lambda ()
      (unless joe-mu4e-user-info
        (load-library (concat user-emacs-directory "mail.el.gpg"))
                      (setq joe-mu4e-user-info t))))
#+END_SRC
